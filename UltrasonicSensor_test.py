...
def test_ultrasonic():
    try:
        PROXITY_TWO_IO_LEFT = UltrasonicSensor.UltrasonicSensor(
            GPIOLayout.SONAR_LEFT_RX_PIN, GPIOLayout.SONAR_LEFT_TX_PIN)
        MODULE_LOGGER.info(
            "Left: " + 
            format(PROXITY_TWO_IO_LEFT.measurement(), '.2f') +
            " cm")
        ...
        MODULE_LOGGER.info(
            "Right: " + 
            format(PROXITY_TWO_IO_RIGHT.measurement(), '.2f') +
            " cm")
        ...
        MODULE_LOGGER.info(
            "Front: " + 
            format(PROXITY_ONE_IO.measurement(), '.2f') +
            " cm")
        PROXITY_ONE_IO.cleanup()

        PROXITY_HIGH_Q = UltrasonicSensor.UltrasonicSensor(
            GPIOLayout.SONAR_FRONT_TX_PIN, qsize=20)
        MODULE_LOGGER.info(
            "Front - Large Queue Average: " + 
            format(PROXITY_HIGH_Q.measurement(), '.2f') +
            " cm")
