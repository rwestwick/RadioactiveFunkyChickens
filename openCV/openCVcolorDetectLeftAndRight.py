...
# resolution defaults to display resolution
...
for frame in camera.capture_continuous(
    ...
    # Capture mean number of white/black pixels in the mask output in each ROW
    # and COLUMN block
    ...
    # Find index of block with largest number of white pixels N.B. Black = 0,
    # White = 255(?)
...
##        print("The HSV blue object is to your left")
    ...
    im2, contours, hierarchy = cv2.findContours(
        mask_hsvImageBlur, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # if the 'c' key was pressed change the colour array counter
        ...
        fileNameMaskBGR = 'bgrImageMask' + \
            str(COLOUR_NAME_ARRAY[colourArrayCntr]) + str(imageNum) + '.png'
        fileNameMaskHSV = 'hsvImageMask' + \
            str(COLOUR_NAME_ARRAY[colourArrayCntr]) + str(imageNum) + '.png'
        fileNameOutputBGR = 'bgrImageOutput' + \
            str(COLOUR_NAME_ARRAY[colourArrayCntr]) + str(imageNum) + '.png'
